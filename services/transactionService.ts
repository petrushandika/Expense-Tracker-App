...
export const createOrUpdateTransaction = async (
  ...
  try {
    ...
    if (image) {
      const imageUplaodRes = await uploadFileToCloudinary(image, "transaction");
    ...
    return {
      success: true,
      data: { ...transactionDate, id: transactionRef.id },
    };
  } catch (error: any) {
...
export const updateWalletForNewTransaction = async (
  ...
  try {
    ...
    if (type == "expense" && walletData.amount! - amount < 0) {
      return {
        success: false,
        msg: "Selected Wallet Don't have sufficent Balance",
      };
    }
